...
export class ForgeService {
    ...
    async authenticate(requestId) {
        ...
        try {
            ...
            childLogger.error({ error: error.message }, 'Forge authentication failed');
            throw createError('Failed to authenticate with Autodesk Forge', 500);
        }
    }
    async ensureValidToken(requestId) {
    ...
    async uploadFileToForge(fileBuffer, fileName, requestId) {
        const childLogger = logger.child({ requestId, service: 'forge' });
        try {
            await this.ensureValidToken(requestId);
            ...
            childLogger.error({ error: error.message }, 'Failed to upload file to Forge');
            throw createError('Failed to upload file to Forge', 500);
        }
    }
    async translateFileInForge(objectId, fileName, requestId) {
        const childLogger = logger.child({ requestId, service: 'forge' });
        try {
            await this.ensureValidToken(requestId);
            ...
            childLogger.error({ error: error.message }, 'Failed to start Forge translation job');
            throw createError('Failed to start Forge translation job', 500);
        }
    }
    async getManifestFromForge(urn, requestId) {
        const childLogger = logger.child({ requestId, service: 'forge' });
        try {
            await this.ensureValidToken(requestId);
            ...
            childLogger.error({ error: error.message }, 'Failed to get Forge manifest');
            throw createError('Failed to get Forge manifest', 500);
        }
    }
    async getPropertiesFromForge(urn, requestId) {
        const childLogger = logger.child({ requestId, service: 'forge' });
        try {
            await this.ensureValidToken(requestId);
            ...
            childLogger.error({ error: error.message }, 'Failed to get Forge properties');
            throw createError('Failed to get Forge properties', 500);
        }
    }
    // Public methods with circuit breakers
    async uploadFile(data, requestId) {
        const childLogger = logger.child({ requestId, service: 'forge' });
        try {
    ...
    async translateFile(data, requestId) {
        const childLogger = logger.child({ requestId, service: 'forge' });
        try {
            ...
            childLogger.error({ error: error.message }, 'File translation failed');
            throw error;
        }
    }
    async getManifest(urn, requestId) {
        const childLogger = logger.child({ requestId, service: 'forge' });
        try {
            ...
            childLogger.error({ error: error.message }, 'Failed to get manifest');
            throw error;
        }
    }
    async getProperties(urn, requestId) {
        const childLogger = logger.child({ requestId, service: 'forge' });
        try {
            ...
            childLogger.error({ error: error.message }, 'Failed to get properties');
            throw error;
        }
    }
    async getTakeoff(urn, requestId) {
...
//# sourceMappingURL=forgeService.js.map