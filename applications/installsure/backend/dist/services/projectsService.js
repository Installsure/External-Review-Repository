...
export class ProjectsService {
    async getAllProjects(requestId) {
        const childLogger = logger.child({ requestId, service: 'projects' });
        try {
            // Try to get from cache first
            const cacheKey = 'projects:all';
            const cachedProjects = await cache.get(cacheKey, { prefix: 'projects', ttl: 300 });
            if (cachedProjects) {
            ...
            childLogger.error({ error: error.message }, 'Failed to fetch projects');
            throw createError('Failed to fetch projects', 500);
        }
    }
    async getProjectById(id, requestId) {
        ...
        try {
            ...
            childLogger.error({ error: error.message }, 'Failed to fetch project');
            throw createError('Failed to fetch project', 500);
        }
    }
    async createProject(data, requestId) {
        const childLogger = logger.child({ requestId, service: 'projects' });
        try {
            ...
            childLogger.info({ projectId: project.id, name: project.name }, 'Project created successfully');
            return project;
        }
        catch (error) {
            ...
            childLogger.error({ error: error.message }, 'Failed to create project');
            throw createError('Failed to create project', 500);
        }
    }
    async updateProject(id, data, requestId) {
        const childLogger = logger.child({ requestId, service: 'projects', projectId: id });
        try {
    ...
    async deleteProject(id, requestId) {
        ...
        try {
            ...
            childLogger.error({ error: error.message }, 'Failed to delete project');
