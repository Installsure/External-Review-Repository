...
router.get('/', cacheMiddleware({ ttl: 300, prefix: 'projects' }), validateBody(paginationSchema), asyncHandler(async (req, res) => {
...
router.post('/', invalidateCache(['projects:*']), validateBody(createProjectSchema), asyncHandler(async (req, res) => {
    const project = await projectsService.createProject(req.body, req.requestId);
    res.status(201).json(project);
}));
// PUT /api/projects/:id - Update project
router.put('/:id', validateParams(idSchema), validateBody(updateProjectSchema), asyncHandler(async (req, res) => {
    const project = await projectsService.updateProject(parseInt(req.params.id), req.body, req.requestId);
    res.json(project);
}));
