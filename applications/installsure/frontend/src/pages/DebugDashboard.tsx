...
export default function DebugDashboard() {
  ...
  useEffect(() => {
    const testConnections = async () => {
      ...
      try {
        ...
        info.backendHealth = { error: err instanceof Error ? err.message : String(err) };
      }
      ...
      try {
        ...
        info.projects = { error: err instanceof Error ? err.message : String(err) };
      }
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-gray-900">InstallSure Debug Dashboard</h1>
        <p className="mt-1 text-sm text-gray-600">Connection testing and debugging information</p>
      </div>

      {/* Backend Health */}
      <div className="bg-white shadow rounded-lg p-6">
        <h3 className="text-lg font-medium text-gray-900 mb-4">Backend Health Check</h3>
            ...
            <div className="mt-2 text-sm text-gray-600">
              ...
              <p>
                ...
                {debugInfo.backendHealth?.data?.uptime
                  ? Math.floor(debugInfo.backendHealth.data.uptime / 60) + ' minutes'
                  : 'Unknown'}
              </p>
              <p>Version: {debugInfo.backendHealth?.data?.version || 'Unknown'}</p>
            </div>
          </div>
        )}
      </div>
      ...
      {/* Projects API */}
      <div className="bg-white shadow rounded-lg p-6">
        <h3 className="text-lg font-medium text-gray-900 mb-4">Projects API Test</h3>
              ...
              {debugInfo.projects?.data?.data && (
                <div className="mt-2">
                  ...
                  <ul className="list-disc list-inside">
                    {debugInfo.projects.data.data.map((project: any, index: number) => (
                      <li key={index}>
      ...
      {/* Raw Debug Data */}
      <div className="bg-white shadow rounded-lg p-6">
        <h3 className="text-lg font-medium text-gray-900 mb-4">Raw Debug Data</h3>
      ...
      <div className="bg-white shadow rounded-lg p-6">
        ...
        <div className="flex space-x-4">
          ...
          <button
            onClick={() => window.open('http://localhost:8000/api/projects', '_blank')}
