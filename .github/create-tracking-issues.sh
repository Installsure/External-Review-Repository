#!/usr/bin/env bash
# Script to create tracking issues using GitHub CLI
# Requires: gh (GitHub CLI) to be installed and authenticated

set -e

REPO="Installsure/External-Review-Repository"

echo "🎯 Creating tracking issues for External Review Repository..."
echo "================================================"
...
# Create labels
gh label create "pri:high" --color "d73a4a" --description "High priority items" --repo "$REPO" 2>/dev/null || echo "  ℹ️  Label 'pri:high' already exists"
gh label create "area:viewer" --color "0075ca" --description "3D viewer and visualization" --repo "$REPO" 2>/dev/null || echo "  ℹ️  Label 'area:viewer' already exists"
gh label create "area:estimator" --color "0075ca" --description "Cost estimation engine" --repo "$REPO" 2>/dev/null || echo "  ℹ️  Label 'area:estimator' already exists"
gh label create "area:ue" --color "0075ca" --description "Unreal Engine integration" --repo "$REPO" 2>/dev/null || echo "  ℹ️  Label 'area:ue' already exists"

echo ""
echo "Creating Issue #1: Boot Scripts & Path Hardening..."

gh issue create \
  --repo "$REPO" \
  --title "Boot Scripts & Path Hardening" \
  --label "pri:high,area:viewer,area:estimator,area:ue" \
  --body "### Overview
...
echo ""
echo "Creating Issue #2: PDF/IFC Tagging Reliability Tests..."

gh issue create \
  --repo "$REPO" \
  --title "PDF/IFC Tagging Reliability Tests" \
  --label "pri:high,area:viewer,area:estimator" \
  --body "### Overview
...
echo ""
echo "Creating Issue #3: Estimator Edge Cases..."

gh issue create \
  --repo "$REPO" \
  --title "Estimator Edge Cases" \
  --label "pri:high,area:estimator" \
  --body "### Overview
...
echo ""
echo "Creating Issue #4: UE5 Pipeline Preflight Checks..."

gh issue create \
  --repo "$REPO" \
  --title "UE5 Pipeline Preflight Checks (UE Path, IFC/Datasmith Availability)" \
  --label "pri:high,area:ue,area:viewer" \
  --body "### Overview
...
echo ""
echo "================================================"
echo "✅ All tracking issues created successfully!"
echo ""
